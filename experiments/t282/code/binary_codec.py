...
def read_model(path:Path, device:str='cpu'):
    ...
    for d in h['sections']:
        ...
        elif kind=='sharedmeta':a=decode_sharedmeta(d,b)
        elif kind=='vq':
    ...
    for d,b in patches:
        if d['name'] not in out:raise ValueError('rowpatch without base tensor')
        base=out[d['name']].cpu().numpy()
        out[d['name']]=torch.from_numpy(apply_rowpatch(base,d,b)).to(device)
...
def encode_rowpatch(a:np.ndarray,rows:list[int]):
    """Private fp16 row exceptions. Rows must be sorted unique valid indices."""
    if a.ndim!=2 or not np.isfinite(a).all():raise ValueError('rowpatch needs finite 2D')
    rows=list(rows)
    if len(rows)!=len(set(rows)) or any(r<0 or r>=a.shape[0] for r in rows):raise ValueError('rowpatch row ids')
    if any(b<a for a,b in zip(rows,rows[1:])):raise ValueError('rowpatch rows must be ascending')
    blob=np.asarray(a[rows],dtype='<f2').tobytes()
    if len(blob)!=2*len(rows)*a.shape[1]:raise ValueError('rowpatch extent')
    return {'kind':'rowpatch','shape':list(a.shape),'rows':rows},blob

def apply_rowpatch(a:np.ndarray,d,blob:bytes)->np.ndarray:
    rows=d['rows'];shape=tuple(d['shape'])
    if (not isinstance(rows,list) or any(type(r) is not int for r in rows)
        or len(rows)!=len(set(rows)) or any(r<0 or r>=shape[0] for r in rows)
        or any(b<a for a,b in zip(rows,rows[1:]))):raise ValueError('rowpatch schema')
    if tuple(a.shape)!=shape:raise ValueError('rowpatch target shape')
    if len(blob)!=2*len(rows)*shape[1]:raise ValueError('rowpatch extent')
    p=np.frombuffer(blob,dtype='<f2').astype(np.float32).reshape(len(rows),shape[1])
    if not np.isfinite(p).all():raise ValueError('nonfinite rowpatch')
    out=a.copy();out[rows]=p
    return out

def encode_sharedmeta(x:np.ndarray,bits:int,group:int=64,supergroup:int=128,importance=None):
    """Shared-metadata uniform codec: codes at `group` granularity, one fp16
    (offset, scale) pair shared per `supergroup` (>group) elements. Viable-regime
    functional sharing: adjacent groups quotient a common scale/offset."""
    if (x.ndim!=2 or x.shape[1]%supergroup or supergroup%group or supergroup<=group
        or not np.isfinite(x).all() or not 2<=bits<=8):raise ValueError('sharedmeta geometry')
    if x.shape[0]*x.shape[1]%supergroup:raise ValueError('sharedmeta length')
    flat=x.astype(np.float32,copy=False).reshape(-1);n=len(flat)
    nsg=n//supergroup;limit=(1<<bits)-1
    if importance is None:importance=np.ones(n,np.float32)
    w=np.asarray(importance,np.float32).reshape(-1)
    if w.shape!=(n,) or not np.isfinite(w).all() or np.any(w<=0):raise ValueError('sharedmeta importance')
    a=flat.reshape(nsg,supergroup).astype(np.float64);ww=w.reshape(nsg,supergroup).astype(np.float64)
    lo=a.min(1);hi=a.max(1)
    scale=np.maximum((hi-lo)/limit,2**-24);off=lo
    for _ in range(3):
        q=np.clip(np.rint((a-off[:,None])/scale[:,None]),0,limit)
        num=(ww*(a-off[:,None])*q).sum(1);den=(ww*q*q).sum(1)
        valid=den>np.finfo(np.float64).eps*np.maximum((ww*(a-off[:,None])**2).sum(1),1.)
        scale=np.where(valid,num/np.maximum(den,1e-300),scale)
        scale=np.clip(scale,2**-24,65504)
        off=((ww*(a-scale[:,None]*q)).sum(1)/ww.sum(1))
        if not np.isfinite(scale).all() or not np.isfinite(off).all():raise FloatingPointError('sharedmeta fit')
    meta=np.stack((off,scale),1).astype('<f2')
    if not np.isfinite(meta.astype(np.float32)).all():raise ValueError('sharedmeta fp16 range')
    m=meta.astype(np.float32)
    q=np.clip(np.rint((a-m[:,0,None])/m[:,1,None]),0,limit).astype(np.uint8)
    codes=pack_bits(q,bits)
    d={'kind':'sharedmeta','shape':list(x.shape),'bits':bits,'group':group,'supergroup':supergroup,
       'n':n,'supergroups':nsg,'meta_bytes':len(meta.tobytes())}
    return d,meta.tobytes()+codes

def decode_sharedmeta(d,blob):
    nsg=d['supergroups'];sg=d['supergroup'];n=d['n'];mb=d['meta_bytes'];bits=d['bits']
    if mb!=nsg*4 or nsg!=n//sg:raise ValueError('sharedmeta shape')
    m=np.frombuffer(blob[:mb],dtype='<f2').astype(np.float32).reshape(nsg,2)
    if not np.isfinite(m).all() or np.any(m[:,1]<=0):raise ValueError('sharedmeta metadata')
    codes=unpack_bits(blob[mb:],n,bits).reshape(nsg,sg).astype(np.float32)
    return (codes*m[:,1,None]+m[:,0,None]).reshape(-1)[:n].reshape(d['shape']).copy()
