...
def rowpatch(a,d,data):
    ...
    if len(data)!=2*len(rows)*shape[1]:raise ValueError('rowpatch extent')
    p=f16(data,(len(rows),shape[1]))
...
def decode_model(path: str|Path, device:str='cpu'):
    ...
    cfg=meta['config'];state={};books={};offset=0;pending=[]
    for d in meta['sections']:
        ...
        if kind=='rowpatch':
            pending.append((d,data));continue
        if kind=='mlp_v2':
            ms=mlp(data)
        ...
        elif kind=='sharedmeta':
            nsg=d['supergroups'];sg=d['supergroup'];n=d['n'];nm=d['meta_bytes'];bits=d['bits']
            if nm!=nsg*4 or nsg!=n//sg or n!=math.prod(d['shape']):raise ValueError('sharedmeta schema')
            m=f16(data[:nm],(nsg,2))
            if np.any(m[:,1]<=0):raise ValueError('sharedmeta scales')
            q=unpack(data[nm:],n,bits).astype(np.float32).reshape(nsg,sg)
            a=(q*m[:,1,None]+m[:,0,None]).reshape(-1)[:n].reshape(d['shape'])
        elif kind=='vq':
        ...
        state[name]=a.copy()
    for d,data in pending:
        if d['name'] not in state:raise ValueError('rowpatch without base tensor')
        state[d['name']]=rowpatch(state[d['name']],d,data)
